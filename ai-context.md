...
export function generateBoneSynapses(tempBonePositions: number[], totalBonePoints: number) {
  ...
  for (let i = 0; i < numBoneSynapses; i++) {
    ...
    const c = new THREE.Color('#ff1b40'); // Glowing bright neon red
    ...
    if (dSeed < 0.5) {
      dc.set('#ffffff'); // brilliant white sparkle cores
    } else if (dSeed < 0.85) {
      dc.set('#ff1b40'); // vivid red
    } else {
...
export function generateBoneConnections(boneSynapses: BoneSynapse[]) {
  ...
  for (let i = 0; i < boneConnections.length; i++) {
    ...
    const dSeed = Math.random();
    if (dSeed < 0.5) {
      dc1.set('#ffffff'); // high-contrast white-red lines
      dc2.set('#ff0000');
    } else {
      dc1.set('#ff1b40');
      dc2.set('#fe8192');
...
export function generateSkullSynapses(tempSkullPositions: number[], totalSkullPoints: number) {
  ...
  for (let i = 0; i < numSkullSynapses; i++) {
    ...
    const c = new THREE.Color('#9c4edd'); 
...
export function generateSkullConnections(skullSynapses: SkullSynapse[]) {
  ...
  for (let i = 0; i < skullConnections.length; i++) {
    ...
    const c1 = new THREE.Color('#7b2dbe');
    const c2 = new THREE.Color('#9c4edd');
  ...
  useEffect(() => {
    ...
    for (let rIndex = 0; rIndex < rows; rIndex++) {
      ...
      for (let cIndex = 0; cIndex < cols; cIndex++) {
        ...
        if (isNearEyeBorder) {
          ...
          else if (blend < 0.7) c.set('#00feff');
          else c.set('#01abff');
        }
    ...
    for (let i = 0; i < numSurfacePoints; i++) {
      ...
      if (foldVal > 0.08) {
        ...
        if (dSeed < 0.7) {
          dc.set('#ffffff'); // dazzling white core
        } else {
          dc.set('#ebfeff'); // shimmering cyan-white
        }
        ...
        if (dSeed < 0.5) {
          dc.set('#0188fe'); // royal diamond blue
        } else if (dSeed < 0.8) {
          dc.set('#e9ccff'); // prismatic violet
        } else {
          dc.set('#1b3dfc'); // deep sapphire refraction
        }
      }
    ...
    for (let i = 0; i < numStemPoints; i++) {
      ...
      if (dSeed < 0.4) {
        ...
        dc.set('#e1ccff'); // crystal lavender
      }
      tempBrainColors.push(c.r, c.g, c.b);
      tempDiamondColors.push(dc.r, dc.g, dc.b);
    }
    ...
    for (let i = 0; i < numSynapses; i++) {
      ...
      const c = new THREE.Color('#ffffff'); // High intense glowing synapse
      ...
      const dSeed = Math.random();
      if (dSeed < 0.6) {
        dc.set('#ffffff'); // hot crystal white
      } else if (dSeed < 0.8) {
        dc.set('#b6feff'); // brilliant electric cyan
      } else {
    ...
    for (let i = 0; i < connections.length; i++) {
      ...
      const c1 = new THREE.Color('#00feff');
    ...
    const outerColor = new THREE.Color('#072c30');
    const maxRadius = 15;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const dSeed = Math.random();
      if (dSeed < 0.1) {
        dc.set('#ffffff'); // brilliant white sparkle
      } else if (dSeed < 0.4) {
        dc.set('#c9faff'); // icy diamond blue
      } else if (dSeed < 0.7) {
        dc.set('#01ffd4'); // bright cyan
      } else if (dSeed < 0.85) {
      ...
      if (aSeed < 0.1) {
        ac.set('#ffffff'); // brilliant white sparkle
      } else if (aSeed < 0.4) {
        ac.set('#fe4c6d'); // glowing pinkish red
      } else if (aSeed < 0.7) {
        ac.set('#fe003d'); // vibrant crimson red
      } else if (aSeed < 0.85) {
        ac.set('#cc0100'); // deep red
      } else {
      ...
      const pSeed = Math.random();
      if (pSeed < 0.1) {
        pc.set('#ffffff'); // brilliant white sparkle
      } else if (pSeed < 0.4) {
        pc.set('#e1aaff'); // glowing lavender orchid
      } else if (pSeed < 0.7) {
        pc.set('#9c4edd'); // neon violet/purple
      } else if (pSeed < 0.85) {
        pc.set('#ff01ab'); // vibrant magenta
      } else {
            ...
            >
              {['Products', 'Features', 'Technology', 'Support', 'Pricing'].map((item) => (
                <div key={item} className="flex items-center space-x-2 md:space-x-3 justify-end hover:text-[#00ffd0] transition-colors cursor-pointer pointer-events-auto">
                  <span>{item}</span>
                  <span className="text-[#00feff] font-bold">•</span>
                </div>
              ))}
            </div>
          </div>
